...
export async function discoverUpdateStateSchemaInspectionInProcess(
  ...
  const snapshot = prepareSqliteReadOnlyLocationSyncInProcess(shared, input.stagingRoot);
  try {
    const sharedVersion = {
      path: shared,
      ...readStateDatabaseVersion(snapshot.location, shared, shared, files),
    };
    // The parent owns stagingRoot. Retain this exact shared generation so the
    // inspection worker cannot rediscover registrations from newer live bytes.
...
async function runUpdateStateInspectionWorker(params: {
  ...
  return await runCommandBuffered(
    ...
    ],
    {
      ...
      env: { XDG_CACHE_HOME: params.stagingRoot },
